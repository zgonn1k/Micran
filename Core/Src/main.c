...
int main(void)
{
	...
	while (1)
	{
		...
		rmsVol = (float)STPMC1_GetMomVoltage(PHASE_T);
/*
		sprintf(UART_TxBuffer, "Mom Voltage = %l\r\n",(uint32_t) STPMC1_GetMomVoltage(PHASE_T));
				SendString(UART_TxBuffer);

		sprintf(UART_TxBuffer, "Mom Current = %l\r\n", (uint32_t)STPMC1_GetMomCurrent(PHASE_T));
						SendString(UART_TxBuffer);

		sprintf(UART_TxBuffer, "RMS Voltage = %l\r\n", (uint32_t)STPMC1_GetRMSVoltage(PHASE_T));
						SendString(UART_TxBuffer);

		sprintf(UART_TxBuffer, "RMS Current = %l\r\n",(uint32_t) STPMC1_GetRMSCurrent(PHASE_T));
						SendString(UART_TxBuffer);
*/
		LL_mDelay(100);
...
void STPMC1_Reading(Data_t *Data)
{
	...
	for (i = 0; i < DATA_SIZE; i++)
	{
		for (j = 0; j < 4; j++)
		{
			...
			dataRegister[i] = (LL_SPI_ReceiveData8(SPI1) << (3-j) * 8) | dataRegister[i];
		}
	}
	...
	{
		sprintf(UART_TxBuffer, "Value from %d register %lu\r\n", i,  dataRegister[i]);
		SendString(UART_TxBuffer);
	}
...
void STPMC1_Data_Init(Data_t *Data)
{
	...
	Data->rmsVoltage[PHASE_T] = (uint32_t) (dataRegister[DET]&VOLTAGE_VALUES_MASK) >> 16;
	Data->rmsVoltage[PHASE_R] = (uint32_t) (dataRegister[DER]&VOLTAGE_VALUES_MASK) >> 16;
	Data->rmsVoltage[PHASE_S] = (uint32_t) (dataRegister[DES]&VOLTAGE_VALUES_MASK) >> 16;
}
